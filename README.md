...
2. ハードウェアの接続<BR>
   1. M5Stackとセンサーを接続<BR>
     M5StackのI2CピンとセンサーのQwiicコネクタを、ブレッドボードとジャンパー線、[Qwiccケーブル](https://www.sparkfun.com/products/14425)などで繋ぎます。<BR>
...
## グラフの歪みについて
  画面が消えている時にボタンを押した時や、CO2通知が表示された時のCO2/TVOCグラフが急に下がる場合がありますが、これはセンサーに供給する電圧が変動した為です。
  本プログラムでは状況によりLCD輝度を変化させていますが、M5StackはLCD輝度を変更すると3.3V端子の出力電圧が微妙に変化するようです。
  CCS811センサーは一定の電圧で使用する仕様なので、途中で電圧を変化させると測定値も変化してしまいます。<BR>
  気になる方は、下記の方法で回避出来ます。
  - お手軽な方法
    LCD輝度を変化させないよう本プログラムを修正することにより、グラフが歪まないようにできます。<BR>
    ★印の行（６か所）をコメントアウトしてください。<BR>
    但し、常に画面が光った状態になります。
  - 完璧な方法
    ブレッドボード上にレギュレータ回路を追加し、M5Stackの5V端子から3.3Vに降圧してからセンサーに入力します。<BR>
    これで、LCD輝度が変化してもグラフが歪まないようになります。
