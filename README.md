...
測定値はM5Stackの液晶画面に、下記３つのモードで表示します。
- 現在値モニタ<BR>
  現在の測定値を表示。
- グラフ<BR>
  過去のデータをグラフ表示。
...
CO2測定値が高い場合は、通知画面を表示し換気を促します。<BR>
...
|1500ppm ～ 2500ppm|オレンジ|悪い|点滅|
|2500ppm ～ 3500ppm|ピンク|非常に悪い|点滅|
|3500ppm ～ 6000ppm|赤|極めて悪い|点滅|
|6000ppm ～|棒グラフ全体が赤くなります|〃|点滅|
...
### CO2通知
  ボタンを押さずに３分経過し画面が消えている間に高いCO2レベルを検知すると、画面が点滅します。
  - １分おきにCO2レベルをチェックしており、設定された値を超えたCO2値を検知すると画面が３秒間点灯します。
  - 既定値では
