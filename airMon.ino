...
void drawGraph(bool chgMode, bool chgView) {
  ...
  //１単位当たりのドット数を計算
  int yScaleMax = (maxCO2 > 3000 ? 6000 : 3000); // 左軸の最大値 (6000を超えるとはみ出る仕様)
  int yScaleInc = yScaleMax / 6;
  static float pDot1 = 222.0 / 140.0;  // 右軸： 温度,湿度 -40～100
  float pDot2 = 222.0 / (float)yScaleMax; // 左軸： 気圧,CO2,TVOC
  ...
  for (int x = 0; x < 320; x += 5) {
    ...
    // 左軸
    for (int val = yScaleInc; val <= yScaleMax - yScaleInc; val += yScaleInc) {
