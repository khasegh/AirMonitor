...
void loop()
{
  ...
  static unsigned long lastReleasedTime = 0; //最後にボタンを離した時間(ms)
  ...
  } else if (!isSleep && millis() - lastReleasedTime > 180000) {
    isSleep = true;
    M5.Lcd.sleep();
  ...
  // 描画
  if (!isSleep && (el.IsUpdated(ed) || chgMode || chgView)) {
    ed = el.getDesc(); //リスト状態の更新
...
void drawMonitor(bool chgMode, bool chgView) {
  ...
  if (ei.CO2 > 6000) {
    ...
    // 3500-6000
    if (ei.CO2 > 3500) {
      int h5 = (118.0 / 6000.0) * ei.CO2 ;
      M5.Lcd.fillRect(241, 109 + (118 - h5), 48, h5, TFT_RED);
    }

    // 2500-3500
    if (ei.CO2 > 2500) {
      int h4 = (118.0 / 6000.0) * (ei.CO2 > 3500 ? 3500 : ei.CO2);
      M5.Lcd.fillRect(241, 109 + (118 - h4), 48, h4, TFT_PINK);
    }

    // 1500-2500
    if (ei.CO2 > 1500) {
      int h3 = (118.0 / 6000.0) * (ei.CO2 > 2500 ? 2500 : ei.CO2);
      M5.Lcd.fillRect(241, 109 + (118 - h3), 48, h3, TFT_ORANGE);
    }

    // 1000-1500
    if (ei.CO2 > 1000) {
      int h2 = (118.0 / 6000.0) * (ei.CO2 > 1500 ? 1500 : ei.CO2);
      M5.Lcd.fillRect(241, 109 + (118 - h2), 48, h2, TFT_YELLOW);
    }

    // 1-1000
    int h1 = (118.0 / 6000.0) * (ei.CO2 > 1000 ? 1000 : ei.CO2);
    M5.Lcd.fillRect(241, 109 + (118 - h1), 48, h1, TFT_GREEN);
  }
}
...
void drawData(bool chgMode, bool chgView) {
  ...
  for (int i = 0; i < max; i++) {
    ...
    M5.Lcd.printf("%7.2fhPa,", ei.pressure / 100); // 1ヘクトパスカル(hPa) = 100パスカル(Pa)
